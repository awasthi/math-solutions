...
\begin{proof}
  ...
  By Cor.~1.4, the irreducible components of $Y \cap H$ correspond to the
  minimal primes of $A(Y \cap H)$, and in particular, the dimension of an
  irreducible component is equal to the coheight of its corresponding minimal
  prime by Thm.~1.8A$(b)$.
  Now $Y \cap H = Z(I(Y) + (f))$ by the proof of Prop.~1.1, hence $I(Y \cap H) =
  \sqrt{I(Y) + (f)}$ by Prop.~$1.2(d)$.
  The minimal primes in $A(Y \cap H) = A/I(Y \cap H)$ correspond to the
  minimal primes in $A$ containing $\sqrt{I(Y) + (f)}$; by
  \cite[Prop.~1.14]{AM69} these are the minimal primes containing $I(Y)
  + (f)$.
  By \cite[Prop.~1.1]{AM69}, these are the minimal primes in $A(Y)$
  containing $f$, which by Krull's Hauptidealsatz (Thm.~1.11A) have height $1$
  hence coheight $r-1$ in $A(Y)$.
  By the reductions above, every irreducible component of $Y \cap
  H$ thus has dimension $r-1$.
\end{proof}
...
\begin{problem} Let $Y \subseteq \AA^3$ be the curve given parametrically by $x = t^3, y= t^4$, $z = t^5$. Show that $I(Y)$ is a prime ideal of height 2 in $k[x,y,z]$ which cannot be generated by 2 elements. We say $Y$ is not a local complete intersection. 
\end{problem}
\begin{proof}
